...
namespace HK.VocationalSchoolAutomason.Api.Controllers
{
    ...
    {
        private readonly IWeek _service;

        public WeekController(IWeek service)
        {
            _service = service;
        }
        ...
        {
            var Response = await _service.GetById<WeekListDto>(id);
            return Ok(Response);
        ...
        [HttpPost]
        public async Task<IActionResult> CreateAsync([FromQuery] WeeKCreateDto dto)
        {
        ...
        [HttpPut]
        public async Task<IActionResult> Update([FromBody] WeekUpdateDto dto)
        {
