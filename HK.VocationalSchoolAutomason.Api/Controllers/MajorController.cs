...
namespace HK.VocationalSchoolAutomason.Api.Controllers
{
    ...
    {
        private readonly IMajor _service;

        public MajorController(IMajor service)
        {
            _service = service;
        }
        ...
        {
            var Response = await _service.GetById<MajorListDto>(id);
            return Ok(Response);
        ...
        [HttpPost]
        public async Task<IActionResult> CreateAsync([FromQuery] MajorCreateDto dto)
        {
        ...
        [HttpPut]
        public async Task<IActionResult> Update([FromBody] MajorUpdateDto dto)
        {
